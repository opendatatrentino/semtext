...
public class SemTextsTest {
	...
	public void testDisambiguate() {
		...
		assertEquals(meaningA,
				SemTexts.disambiguate(ImmutableList.of(meaningA, Meaning.of("b", MeaningKind.ENTITY, 0.2))));

		assertEquals(null, SemTexts.disambiguate(
				ImmutableList.of(Meaning.of("a", MeaningKind.ENTITY, 0.2), Meaning.of("b", MeaningKind.ENTITY, 0.2))));
	...
	public void checkMergeMeanings() {
		...
		ImmutableList<Meaning> ms = SemTexts.mergeMeanings(ImmutableList.of(m1), ImmutableList.of(m2));
	...
	public void testTerm() {
        ...
        try {
            SemTexts.term(SemText.of());
            Assert.fail("Shouldn't arrive here!");
        } catch (SemTextNotFoundException ex) {

        }
                
	    Term term1 = Term.of(0, 1, MeaningStatus.NOT_SURE, null);
	    Term term2 = Term.of(1, 2, MeaningStatus.NOT_SURE, null);
         
        assertEquals(term1, SemTexts.term(SemText.of(
                            Locale.ITALIAN, 
                            "abc", 
                            term1
                            )));
    ...
    public void testMeaning() {
        ...
        try {
            SemTexts.meaning(SemText.of());
            Assert.fail("Shouldn't arrive here!");
        } catch (SemTextNotFoundException ex) {

        }
                
        Meaning m1 = Meaning.of("a",MeaningKind.CONCEPT,0.5);
        Term term1 = Term.of(0, 1, MeaningStatus.SELECTED, m1);
        Term term2 = Term.of(1, 2, MeaningStatus.NOT_SURE, null);
         
        assertEquals(m1, SemTexts.meaning(SemText.of(
                            Locale.ITALIAN, 
                            "abc", 
                            term1
                            )));
    ...
    @Test
    public void testMeaningKind() {
        
        try {
            SemTexts.meaningKind(SemText.of());
            Assert.fail("Shouldn't arrive here!");
        } catch (SemTextNotFoundException ex) {
        ...
        assertEquals(MeaningKind.CONCEPT, SemTexts.meaningKind(SemText.of(
                            Locale.ITALIAN, 
                            "abc", 
                            term1
                            )));
